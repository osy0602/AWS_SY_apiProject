...
#json server에 있는 data 받아오기 설계서 씀
...
#json data mongo insert 직구 데이터 몽고 디비에 인서트하기 설계서 씀!
...
#!!!!!!!!!!!!
#cross border mongo data get test용임
@app.get(path='/get')
async def get():
    col = db['cross_border']
...
#!!!!!!!!!!!!!!
#crossborder mongo data delete test용 데이터임
...
# year | subject input 후 해당하는 data mongo에서 가져오기 설계서 씀
@app.get(path='/shop')
async def selectShop(year=None, subject=None):
    # result= {"resultcode": response_code}
    # mongodb에 데이터 넣어주는 작업 먼저 해주기
...
#!!!!!!!!!!!!!!!!!!!!!
# SQL test용 api jikgu table에 있는 data 가져오기
@app.get('/getJikguAll')
async def getAll():
    result = session.query(Jikgu)
    return result.all()

# sql에 상품군별 insert하기 설계서 씀
@app.get('/insertSQL')
...
# graph data sql로 insert하기 사실 안해도 되긴 함,,
@app.get('/insertImage')
...
@app.get('/getImageinSQL')
async def getImageinSQL():
    result = session.query(JPData).first().image
    resultImage = result
    # print(result.first().image)
    # return 0
    # return render_template("calcTest.html",image_data=resultImage)

# 그래프 유사율 데이터 SQL에 저장하기
...
# 현주한테 줘야하는 api 년도별 합계 직구 데이터 return
@app.get('/usingData')
async def usingData():
    col = db['cross_border']
